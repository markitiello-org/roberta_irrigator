...
class PersistentLogDAO:
    ...
    def AddLog(log:Log):
        ...
        SqlLite.get_instance().ExecuteQueryNoResult(
            ...
            [log.zone_id, log.date_time.strftime("%Y-%m-%d %H:%M:%S.%f"), int(log.event_id.value), log.log]
        )

    @staticmethod
    def GetLogs(zone_id:int=None, event_id:EventId=None, number_of_logs_to_get:int=None):
        ...
        if event_id is not None:
            ...
            params.append(int(event_id.value) if hasattr(event_id, "value") else event_id)
        ...
        for row in data:
            print (row[1])
            logs.append(Log(row[0], datetime.datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S.%f"), EventId(row[2]), row[3]))
        return logs
